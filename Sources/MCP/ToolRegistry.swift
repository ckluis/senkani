                ...
                schema: Tool(
                    ...
                    inputSchema: .object([
                        ...
                        "properties": .object([
                            ...
                            "dispatch": .object(["type": .string("string"), "enum": .array([.string("subprocess"), .string("headless"), .string("pane")]), "description": .string("U.2b-2 — runner selector. 'subprocess' (default) drives the node+Playwright Chromium subprocess. 'headless' drives the off-screen WKWebView runner registered via BrowserDispatchRegistry by the SenkaniApp host at startup; without that registration (standalone senkani-mcp binary) the dispatcher returns a structured headless_not_yet_implemented refusal. 'pane' targets a visible BrowserPane (see pane_id) via the pane runner registered by the SenkaniApp GUI half; without that registration (standalone senkani-mcp binary / GUI not running) the dispatcher fails closed with a structured validation_browser_pane_no_runner refusal — never a silent pass. Audit-chain rows carry runner=<value> in all three modes (subprocess → 'subprocess'; headless → 'wkwebview-headless'; pane → 'wkwebview-pane').")]),
