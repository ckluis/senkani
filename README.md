...
| `senkani_bundle` | Budget-bounded repo snapshot. Local mode composes symbol outlines + dep graph + KB entities + README in a canonical truncation-robust order. Remote mode (`remote: "owner/name"`) snapshots any public GitHub repo via `senkani_repo` — same host allowlist + `SecretDetector`. Emits `format: "markdown"` (default) or stable-schema `format: "json"` (`BundleDocument`). `preview: true` (U.10a) returns a `ContextManifest` review surface (lanes + modes + per-item token estimates) instead of the body; `modes:` and `lanes:` narrow the preview. U.10a-2 secret gate: preview is refused when any item carries a `SecretDetector` hit unless `allow_secrets: true` is passed — every override writes a chained `bundle.secret.allow` audit row, every preview call writes a chained `bundle.dispatch` audit row. Path-validated. | repo-level |
