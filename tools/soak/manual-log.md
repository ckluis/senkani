...
- **Cross-walk note (groom 2026-05-22):** this walk's `mcp-driver.py` heredoc uses the bare wire name `validate_browser` (NOT `senkani_validate_browser`) and injects `SENKANI_PANE_ID` via `env.setdefault(…, str(uuid.uuid4()))` so the MCP server's access gate activates. The U.2b-1b-4 + U.2b-1b-5 walks above (groomed 2026-05-22) carry latent defects on both points — see follow-up [`process-gap-walks-mcp-driver-corrections-2026-05-22`](../../spec/autonomous/backlog/process-gap-walks-mcp-driver-corrections-2026-05-22.md). Cowork / operator should run THIS walk's driver as the canonical pattern until that follow-up closes.
...
- **Validates:**
  - **(a) MCP transport health (Step 1)** — `.build/debug/SenkaniApp --mcp-server`'s `tools/list` enumerates `senkani_validate_browser`; the `tools/call` against `http://127.0.0.1:$FIXTURE_PORT/design-roundtrip.html` with `dispatch=headless` returns a structured `PlaywrightResult` (not `headless_not_yet_implemented` — which would mean the factory wasn't registered).
...
- **project-and-workstream-remove-ui-validation-walk-2026-05-14 — Cowork-runnable real-machine validation walk of the new sidebar destructive-action contract (hover X, tiered-checkbox dialogs, create-time disclosure)** ([groomed plan 2026-05-14](../../spec/autonomous/backlog/project-and-workstream-remove-ui-validation-walk-2026-05-14.md), [parent build round](../../spec/autonomous/completed/2026/2026-05-14-project-and-workstream-no-remove-ui-2026-05-14.md)). Exec mode: **cowork end-to-end** with one operator-only hover sanity-check at step 28 (the one area where Cowork-driven synthetic mouse motion may diverge from real-operator-mouse hover delivery). Time estimate: **~35-45 min operator-supervised** (8 min shell + 25-35 min GUI + 1 min hover sanity). 28-step plan (+ step 18b cleanup) covers all 14 acceptance bullets: hover→X→sheet on project + workstream rows; dialog layout (disabled-checked Sidebar entry, default-checked app-support row with resolved path, per-child-workstream toggle pairs); yellow ⚠ on unpushed/no-upstream branches; greyed-out missing worktree toggle; red Confirm button; Esc dismiss-no-side-effects; documented removal order (branches → worktrees → app-support → sidebar) verified by filesystem inspection; failure surfacing + force opt-in checkbox; workstream X suppression on default + lone; NewWorkstreamSheet disclosure block + branch override + non-git degraded note. Pre-condition: working tree on `main`, no running SenkaniApp (`pgrep -fl SenkaniApp` empty per 2026-05-05 process standard), bundle fresh via `senkani walk rebuild-bundle <bundle>` (or `senkani doctor` check #20 green), `~/.senkani/workspace.json` backed up + cleared (Setup script does this idempotently; Teardown restores). Fixture: throwaway projects under `/tmp/senkani-walk-remove-ui-<RUN_TS>/projects/` — projA (git + remote), projB (git + remote, will accumulate one unpushed/no-upstream branch + dirty worktree), projC (non-git). Audit-accepted risks (Norman/Torvalds/Kleppmann groom): hover-event delivery (covered by operator sanity check step 28); workspace.json hardcoded path no env override (covered by backup/restore); unpushed-warning may surface as `.unpushed(count:)` OR `.noUpstream` (dialog renders ⚠ for both — plan accepts both). **Pre-grooming finding (filed 2026-05-14 as `remove-retry-sidebar-mutation-leak-2026-05-14`):** the force-retry-after-failure path on `WorkspaceModel.removeWorkstream` (mirror in `removeProject`) is structurally broken — in-memory sidebar mutation runs unconditionally on the first call, so the retry's `guard let ws` short-circuits and the worktree dir + branch leak on disk. Step 18 is adjusted to observe the leak as the current-build PASS signal; if the filed bug ships before the walk runs, the verifier flips to the post-fix expectation. Walk does not gate the bug fix; the filed item is the gate. Groomed 2026-05-14 by `senkani-autonomous`; awaits operator/Cowork execution → status: done → next `/senkani-autonomous` close-mode finalize.
