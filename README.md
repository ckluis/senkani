...
- **Autorun loop (Phase U.3, leg 1)** — `senkani autorun --tasks <path> [--dry-run]` runs a free-form markdown task file (one top-level bullet per task) as a one-task-one-commit loop with a per-task validation gate. A `TaskDecomposer` converts each bullet into a U.11 `WorkstreamTaskContract` (inline `[cmd: …]` / `[files: …]` annotations, default test/lint gate otherwise) and persists the plan to `~/.senkani/autorun/<run-id>/contracts.json` (atomic, canonical JSON) as a resume seam. Each task's gate commands run with CI-style short-circuit through the existing `ValidationStore` (a "clean" row per command, scoped to a unique `autorun-<run-id>` session); all-clean commits and emits a `notifyDone`, any failure HALTS the loop with a `notifyFailure`. `--dry-run` prints the decomposed plan + per-task gate and exits without executing anything. An unattended run (stdin not a tty) refuses unless Pushover is seeded, naming `senkani doctor --seed-pushover-key`. The TUI / decomposer pane, `--supervise-first`, `ctrl+.` WIP-stash halt, `--allow-classes` taskClass allowlist, and the real-device Pushover push are later legs.
