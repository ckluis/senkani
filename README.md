...
**SkillPacks** (Phase V.11a) bundle multiple HandManifest skills with a HookRouter policy fragment and a context doc into one unit-of-distribution. `senkani pack install <dir>` copies the pack tree under `~/.senkani/packs/<name>/`, runs collision detection on skill names + policy scope-keys + context filenames (refuses without `--force`), and writes a chained `pack_audits` row provenance-tracked through T.5. `senkani pack uninstall <name>` reverses the operation with zero residue and a chained uninstall row. `senkani pack list` surfaces installed packs + version. The first in-tree pack lives at `spec/packs/code-quality/`; V.11b adds `security` and `devops` plus an integration test. Format frozen at `spec/skill_packs.md`.
