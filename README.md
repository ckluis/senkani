...
| `senkani_bundle` | Budget-bounded repo snapshot. Local mode composes symbol outlines + dep graph + KB entities + README in a canonical truncation-robust order. Remote mode (`remote: "owner/name"`) snapshots any public GitHub repo via `senkani_repo` — same host allowlist + `SecretDetector`. Emits `format: "markdown"` (default) or stable-schema `format: "json"` (`BundleDocument`). `preview: true` (U.10a) returns a `ContextManifest` review surface (lanes + modes + per-item token estimates) instead of the body; `modes:` and `lanes:` narrow the preview. U.10b modes ship live: `slice: "<path>:<start>:<end>"` adds a file-lane slice item with the resolved content; `diff: "<selector>"` (`unstaged`/`staged`/`branch:<ref>`/`range:<a>..<b>`) shells `git diff` and emits one diff-lane item per affected file; `summary` mode emits per-entity Gemma summaries (falls back to `KnowledgeEntity.compiledUnderstanding`, or `summary_unavailable` if neither is present). U.10a-2 secret gate: preview is refused when any item carries a `SecretDetector` hit unless `allow_secrets: true` is passed — every override writes a chained `bundle.secret.allow` audit row, every preview call writes a chained `bundle.dispatch` audit row. Path-validated. | repo-level |
...
| `senkani_search_web` | Web search via DuckDuckGo Lite (no key, no quota). Returns `{title, url, snippet}` triples. Host-pinned to `lite.duckduckgo.com`, SSRF-guarded, redirect-pinned, every snippet + title scanned by `SecretDetector`. `guard-research` denies queries containing absolute paths, globs, or secret-shaped tokens. CAPTCHA / soft-block pages auto-rotate to the next region in `[primary, wt-wt, us-en, uk-en, de-de]` within the same call (bounded 2 HTTP attempts) and persist a 30-min cool-down to `~/.senkani/state/search_web_cooldown.json` (mode 0600); when every region is cooling the call returns a structured `allRegionsCooling` error naming each expiry. Snippets are adversarial third-party text — pass URLs through `senkani_web` before acting. | query-level |
...
**SkillPacks** (Phase V.11a–V.11b) bundle multiple HandManifest skills with a HookRouter policy fragment and a context doc into one unit-of-distribution. `senkani pack install <dir>` copies the pack tree under `~/.senkani/packs/<name>/`, runs collision detection on skill names + policy scope-keys + context filenames (refuses without `--force`), and writes a chained `pack_audits` row provenance-tracked through T.5. `senkani pack uninstall <name>` reverses the operation with zero residue and a chained uninstall row. `senkani pack list` surfaces installed packs + version. Three in-tree packs ship under `spec/packs/`: `code-quality` (denies lint-suppression markers — `eslint-disable-next-line`, `swiftlint:disable`, `# noqa`), `security` (denies hard-coded `API_KEY=`, `password = "` literals, `eval(` invocations), and `devops` (denies `kubectl delete`, `terraform destroy`, `--context=prod` targeting). HookRouter merges installed pack policy fragments live: `PackInstaller` re-reads on `apply()` / `uninstall()`, and the daemon re-reads on every hook event whose install-root mtime has advanced — no app restart required between install and policy-firing. Format frozen at `spec/skill_packs.md`.
...
- **Agent Timeline** — timeline of optimization events, interactive tool calls, and scheduled-task runs (start/end/blocked). Rows whose `session_id` correlates to OTLP spans with `status_code = ERROR` or `duration > p99` show a runtime-error badge dot (V.18a-7); first click expands a compact inline summary, second click opens a side pane that loads the full span tree on demand
...
- **Schedules** — manage recurring tasks via launchd. The New Schedule form composes a cadence three ways — **prose** ("every weekday at 9am", compiled live), **counter** ("every 10 tool_calls"), or **cron** (preset or custom) — with a live `AmplificationGuard` verdict banner, a `CronPreview` next-fires preview, and a Create button gated on an OK verdict (amplifying schedules need an explicit override). Ships with five day-1 presets (`log-rotation`, `morning-brief`, `autoresearch`, `competitive-scan`, `senkani-improve`) installable from the pane or via `senkani schedule preset install <name>`; each preset's resolved command is secret-scanned before install, and missing companion prerequisites (Ollama daemon, `guard-research` hook preset, `senkani brief` CLI, …) surface as warnings, not blockers. Existing schedules support **edit-in-place** (selecting one reopens the compose surface prefilled with its current cadence), **drag-to-reorder** (persisted via `ScheduleStore`), and **inline validation tooltips** surfacing the `AmplificationGuard` rationale and any compile error.
- **Autorun loop (Phase U.3, legs 1–4)** — `senkani autorun --tasks <path> [--dry-run] [--supervise-first N] [--allow-classes <list>]` runs a free-form markdown task file (one top-level bullet per task) as a one-task-one-commit loop with a per-task validation gate. A `TaskDecomposer` converts each bullet into a U.11 `WorkstreamTaskContract` (inline `[cmd: …]` / `[files: …]` annotations, default test/lint gate otherwise) and persists the plan to `~/.senkani/autorun/<run-id>/contracts.json` (atomic, canonical JSON) as a resume seam. Each task's gate commands run with CI-style short-circuit through the existing `ValidationStore` (a "clean" row per command, scoped to a unique `autorun-<run-id>` session); all-clean commits and emits a `notifyDone`, any failure HALTS the loop with a `notifyFailure`. `--dry-run` prints the decomposed plan + per-task gate and exits without executing anything. `--supervise-first N` (leg 2) pauses for an operator y/n AFTER the gate clears and BEFORE the commit-event on the first `N` tasks — `n` aborts the run (distinct from a gate halt), tasks at index ≥ N run unattended; it defaults to `0` (fully unattended) so CI / non-interactive / dry-run stay safe, and refuses up front when no operator is on the TTY. The supervision question arrives through an injected `SupervisionPrompt` seam so the Core spine never reads stdin (headless-tested). An unattended run (stdin not a tty) refuses unless Pushover is seeded, naming `senkani doctor --seed-pushover-key`. `--allow-classes <comma-list>` (leg 3) restricts the loop to tasks whose inferred `TaskClass` (a whole-word keyword heuristic — `test-fix`/`bug-fix`/`feature`/`docs`/`refactor`/`chore`/`unknown`, stamped per bullet) is on the list; a task whose class is off a non-empty list pauses for an operator ack regardless of `--supervise-first` (an empty list = allow-all, an unrecognized/`unknown` class is never auto-allowed — fail-safe). The class rides in the `contracts.json` envelope only (no DB column). Leg 4 wires the real seed-gated Pushover transport: an `AutorunSinkSelector` runs a VALUE-FREE (`CredentialVault.list`, never `read`) FAIL-CLOSED probe for the seeded credential and, when present, installs the real `KeychainPushoverTransport` (otherwise an inert sink) — the same probe drives the unattended-refusal gate, replacing leg 1's fake-transport stopgap. It is plumbing: the branch stays inert until the operator seeds the credential and the real macOS Keychain (T.4c) lands. The TUI / decomposer pane, the real-device Pushover push proof (operator leg C), and first-run approval are later legs (`ctrl+.` WIP-stash halt is disqualified — the loop never mutates the working tree).
...
- **Release commitments — five shape numbers per release.** Beyond the runtime SLOs above, every release reports cold-start (< 250 ms p95), idle memory (< 75 MB), install size (< 50 MB), classifier latency (< 2 ms p95, slot pending U.1 TierScorer), and OpenAI-endpoint cold-start (< 1500 ms p95 — `senkani serve --openai` spawn → listener-ready, Phase V.13e-3). Capture is `tools/measure-slos.sh`, which appends a row to `~/.senkani/slo-history.jsonl` with `git_sha` + `version` for trend correlation. `senkani doctor` reads the latest row and runs a median-of-5 baseline regression check — anything ≥10% over baseline fails the gate. Improvements never fail.
...
- **Tamper-evident audit chain (Phase T.5).** Every row in fourteen chained tables — `token_events`, `validation_results`, `sandboxed_results`, `commands`, `pane_refresh_state`, `confirmations`, `trust_audits`, `policy_snapshots`, `egress_decisions`, `pack_audits`, `eval_results`, `surrogate_writes`, `workstream_handoffs`, and `openai_request_log` (introduced across migrations v4 / v5 / v6 / v11 / v12 / v17 / v19 / v20 / v24 / v27 / v40 / v41, listed migration-chronologically so future additions append rather than re-sort) — carries `prev_hash` + `entry_hash` + `chain_anchor_id`. `entry_hash = SHA-256(prev_hash || canonical_row_bytes)` — a single-byte tamper at row N invalidates row N's hash and every subsequent row's hash. `senkani doctor --verify-chain` walks every chain and reports OK or names the first broken `(table, rowid)`; the integrity line surfaces in `senkani doctor` as `chain integrity: OK since <ISO-date> / N repairs`. Recovery via `senkani doctor --repair-chain --table <T> --from-rowid <N>` opens a fresh chain segment with typed-string double-confirm, tty enforcement, and prior-tip linkage in the new anchor's `operator_note`. The repair surface covers every integer-keyed chain participant; `sandboxed_results` (TEXT primary keys) is repaired by hand pending an `--from-created-at` flag.
...
- **Notification sinks (Phase T.6b + T.6 production hookup, 2026-05-21).** `StdoutSink` writes one canonical JSON line per `NotifyEvent` (sorted-key, scalar-only payload + ISO-8601 `ts`) — pipe `senkani` stdout into a structured-log collector and the gate's outcomes round-trip cleanly. `MacOSLocalSink` posts banners via an injected `LocalNotifierBridge`; SenkaniApp now wires the real `UNUserNotificationCenter`-backed `UNNotifierBridge` at App boot (requests TCC authorization on first launch) while CLI / MCP / CI run silent via `NullLocalNotifierBridge`. CI tests use `SpyLocalNotifierBridge`. `NotificationRouter` reads `~/.senkani/notifications.json` (`{"sinks": {"stdout": {"events": ["notify_failure"]}}}`) to pick which sink fires for which event variant — defaults to subscribe-all when the file is missing. Producers fan out through the process-global `NotificationDelivery` holder, which is `nil` (silent) in CLI / MCP / test processes and only non-nil once SenkaniApp's `NotificationBootstrap` runs. The first production producer fires on the first-ever `firstNonzeroSavings` milestone ("Save your first tokens") via `OnboardingMilestoneStore.record`. The **Settings → Notifications matrix** (2026-06-11) is the sidebar's Notifications tool view: a per-sink × per-event grid of checkboxes that edits `~/.senkani/notifications.json` through the deterministic `Config` writer and live-reloads the router on every flip (no app restart) — plus test-fire buttons that push a synthetic event through the live router. The Pushover adapter's Keychain transport is built; its live wiring (real token seed + egress allow rule) is the remaining T.6c operator leg.
- **EgressProxy live listener (Phase T.1a + T.1a.2 + T.1b + T.1c + T.1b-followup).** `EgressRuleEngine` matches hosts against an exact / prefix / suffix / single-`*` glob rule set with deny-wins precedence and default-deny-on-miss; `EgressHostNormalizer` canonicalizes case, default ports (`:80` / `:443`), trailing dots (T.1c added the second post-port-strip dot pass so `EXAMPLE.com.:80` canonicalizes cleanly), and trailing slashes so a rule for `example.com` matches `Example.COM:80.` exactly the same way; `EgressDecisionStore` chains every decision into the `egress_decisions` audit table (migration v19) so `senkani doctor --verify-chain` proves no allow/deny was redacted. `EgressListener` binds a POSIX TCP socket to `127.0.0.1:<port>` (port 0 = kernel-assigned, persisted to `~/.senkani/egress.port` via atomic rename), and `EgressConnectionHandler` reads the request line, evaluates the host through `EgressRuleEngine`, and either pipes to the upstream (plain HTTP rewritten to origin form, CONNECT tunnels with peeked TLS ClientHello SNI validated against the CONNECT host) or replies `403 Forbidden`. SNI mismatch writes a deny row with sentinel `rule_id="sni_mismatch"` and tears down without piping a single byte upstream. **T.1b layers in** a `PaneMode` taxonomy (`research`, `write`, `redteam`, `general` — typed enum, parsed from an internal `X-Senkani-Pane-Mode` proxy header stripped before forward), per-mode allowlists loaded from `~/.senkani/egress-policy.json` (with fallback to the legacy flat `egress-rules.json` for T.1a operators), and a Gemma judge fallback on static-rule miss (`JudgeAdapter` protocol with `GemmaJudgeAdapter` enforcing a 300 ms hard timeout that defaults to `.deny` on expiry — Schneier P0) whose rationale lands in `egress_decisions.judge_rationale` (migration v23 with per-anchor chain shape switch via `fresh-install-pre-v23` rename + `migration-v23` anchor opened at `MAX(id)`). `.redteam` panes deny on every static-miss **without** invoking the judge — verified by inference-call counter, not by timing (Vitalik posture: no model layer can be asked to allow exfiltration from a redteam pane). **T.1c hardens the defense** with a 20-scenario adversarial corpus (DNS rebinding ×4, redirect chain to private IP ×4, SSRF ×4, CONNECT-vs-SNI mismatch ×2, allowlist boundary ×4, judge-prompt-injection ×2) that runs as part of every `swift test`, plus a `senkani doctor --check-egress` 5-scenario smoke subset (engine-level, 11 ms wall-clock, exits non-zero on any miss) operators (and CI) can run on demand. **T.1b-followup ships the write side**: `PaneModeStore` (JSON at `~/.senkani/pane-modes.json`, NSLock-guarded cache, silent `.general` default on missing-or-malformed file) backs `HookRouter.paneModeResolver`, and `PaneLaunchEnv` injects `SENKANI_PANE_MODE=<mode>` into every pane subprocess env so senkani-aware HTTP clients can emit the matching `X-Senkani-Pane-Mode` header on outbound traffic — proven end-to-end by a redteam integration test that spawns a `/bin/bash -c` subprocess with the env set, splices `$SENKANI_PANE_MODE` into the request header via `printf`, and asserts the daemon records `pane_mode == .redteam` while `MockJudgeAdapter.callCount` stays at `0`. `senkani egress start|stop` daemonize / signal via `~/.senkani/egress.pid`; `senkani egress status [--recent N]` reports listener state + decision count; `senkani doctor` flips to "Egress proxy: running on :PORT (decisions: N)" once the listener is up. HTTPS body MITM (T.1d) is tracked separately.
- **Credential gateway injection (Phase T.4b).** `MCPToolConfig.credentialGateway` declares which `vaultKeys` a tool needs at which `scope`, with a per-tool `dryRun` opt-in for `FAKE_KEY` placeholders. `HookRouter` consults `CredentialGateway` after the confirmation gate (so a `.deny` short-circuits before any vault read), fetches each declared key from `CredentialVault.shared`, and on a missing key + `dryRun: false` returns a structured deny carrying both keyname AND scope so the operator can run `senkani vault add` without guessing. The audit row written to `token_events` records keyname + scope + the dry-run flag — the credential value itself is never serialized. Round T.4b ships the API surface against `InMemoryKeychainStore`; the macOS Keychain conformance for *secrets* and adversarial corpus eval land in T.4c. The first `senkani vault` verb shipped early in V.13a-2 — `senkani vault add openai-key` (below) — backed by a 0600 JSON store of verifier *hashes* (not secrets), so it does not wait on the T.4c Keychain work.
- **OpenAI-compatible endpoint bearer auth (Phase V.13a-2).** `senkani serve --openai` (the V.13a-1 `NWListener` scaffold) gains bearer-token auth on the `/v1/*` prefix, enforced *before* the not-yet-wired `501` surface. `senkani vault add openai-key --preset <auto|build|research|quick|local> [--scope chat,embeddings] [--rate <rpm>] [--expires <iso8601>] [--label <name>]` provisions an `sk-senkani-…` key, prints the plaintext **once** to stdout, and persists only its SHA-256 hash. `--preset` is the routing tier the key uses (a `ModelPreset` case, validated at provision time — a provider name like `openai` is rejected with the valid-cases list). `OpenAIAuthGate` enforces the contract: missing/malformed/unknown/expired key → `401`, out-of-scope surface → `403`, over the per-key rate limit (default 60 rpm) → `429` + `Retry-After`. Key matching is a constant-time, no-short-circuit hash compare that loops every record — "unknown key" and "wrong key" are timing-indistinguishable and return the same `401`. A key provisioned (or revoked) while `serve --openai` is already running takes effect on the *next request* — no restart (the listener reads a live `(mtime, size)`-gated snapshot of `~/.senkani/openai-keys.json`); and that disk store is sealed to its non-secret scope, refusing to persist secrets under any other scope. Streaming chat / embeddings / tool-use surfaces land in v13b … v13d.
- **OpenAI-compatible chat (non-streaming) + preset routing + audit (Phase V.13a-3).** `POST /v1/chat/completions` returns a strict OpenAI `chat.completion` object (`id` / `object` / `created` / `model` / `choices[].message` / `usage`). The provisioned key's `ModelPreset` **wins** routing through `ModelRouter` — the request's `model` (e.g. `gpt-4o`) is logged for telemetry only and never changes routing, and the response's `model` reports the *actual* tier model (Quick → `claude-haiku-3.5`). Every request appends one tamper-evident audit-chain entry (`{ ts, key_label, surface, model_logged, preset_used, resolved_tier, prompt/completion_token_count, status }`, hashed with the Phase T.5 `ChainHasher`); request/response bodies are stored only under `--audit-bodies` (default off). Array-valued `content` returns `400`; `stream: true` opens an SSE stream (Phase V.13b, below). This child ships the routing + audit surface; the real local-Gemma chat backend now backs `.local` end-to-end (Phase V.13 chat-backend-1…4, below).
- **OpenAI-compatible chat streaming (SSE) (Phase V.13b).** `POST /v1/chat/completions` with `stream: true` emits Server-Sent Events (`Content-Type: text/event-stream`) carrying the OpenAI `chat.completion.chunk` delta shape — a leading `{role: "assistant"}` chunk, incremental `{content: …}` chunks (which concatenate byte-for-byte back to the full message), a terminal `finish_reason: "stop"` chunk, then the final `data: [DONE]` sentinel. Rate limiting is enforced *before* the stream opens (a `429` is a complete framed response, never a half-open SSE); a client disconnect cancels the in-flight stream on the next chunk write; and a streamed request still appends exactly one tamper-evident audit-chain entry whose `status` distinguishes completion (`ok`) from client-cancel (`client_cancel`). `OpenAIChatStream` (`Sources/Core/OpenAIEndpoint/OpenAIChatStream.swift`) holds the `Network`-free framing + drive loop; the live `NWConnection` writer streams the chunks and tears the connection down exactly once.
- **OpenAI-compatible embeddings (Phase V.13c).** `POST /v1/embeddings` returns a strict OpenAI `list` object — `data[]` of `{ object: "embedding", index, embedding }` plus `usage` (`prompt_tokens` / `total_tokens`). The `input` field accepts a single string or an array of strings; token-id forms and an empty input are rejected with `400`. The embedding model is sourced from `ModelManager` (the single `minilm-l6` registry entry the MCP `senkani_embed` tool already uses — `EmbedEngine.modelId` now references `ModelManager.embeddingModelID`, so there is no parallel embedding stack), and the response reports that model. A key without `embeddings` in scope gets `403`; each request appends exactly one tamper-evident audit-chain entry with `surface: "embeddings"` and no input text. `OpenAIEmbeddingsHandler` (`Sources/Core/OpenAIEndpoint/`) is the pure, fully-testable surface; real on-device MiniLM-L6 inference now backs the surface when the MCP target is active (via an `EmbeddingEngine` protocol seam + `ModelManager.registerEmbeddingHandler`), with a `model_not_available` framed 503 when the model isn't downloaded — no auto-pull on the hot path.
- **OpenAI-compatible tool-use / function-calling (Phase V.13d).** `POST /v1/chat/completions` now accepts `tools: [{ type: "function", function: { name, description, parameters: <JSON-Schema> } }]` and emits `tool_calls` when the model calls a tool — `choices[].message.tool_calls[].function.{name, arguments}` with `content: null` and `finish_reason: "tool_calls"`, where `arguments` is a JSON-encoded **string** (matching OpenAI exactly). A tool's `parameters` JSON-Schema round-trips verbatim (new `JSONValue`), and a multi-turn `[user → assistant(tool_calls) → tool(result)]` context is accepted and resolves to a text answer. The declared tools bridge through senkani's existing `MCPToolConfig` catalog (new `OpenAIToolBridge`, function tools → `.exec`) — **no parallel tool registry**. The `tools` scope is body-derived, so it is enforced in the handler: a key without `tools` in scope gets `403 insufficient_scope` even with a valid `tools` payload (a streaming tool-use request cannot bypass it), and a malformed declaration → `400`. Each tool-use request still appends exactly one tamper-evident audit-chain entry. Tool calls are produced by the placeholder engine until the real model backend lands; SSE `delta.tool_calls` streaming shipped in V.13d-1 (below).
- **Real on-device Gemma 4 backs `POST /v1/chat/completions` (Phase V.13 chat-backend-1, 2026-05-30).** When the MCP target is active, `senkani serve --openai` resolves a registered `ChatEngine` (new pure-Swift protocol in `Sources/Core/OpenAIEndpoint/ChatEngine.swift` + a `ModelManager.registerChatHandler(_:)` slot mirroring the V.13c embedding-handler pattern) and routes chat completions through `OpenAIChatServeBridge.syncEngine(...)` — a `DispatchSemaphore`-based sync wrapper over the async chat call so the existing `NWListener` closure signature stays unchanged. The MCP-side `MLXChatEngineAdapter` (`Sources/MCP/ChatHandlerRegistration.swift`) loads the largest Gemma 4 tier that fits in available RAM (`gemma4-26b-apex` → `gemma4-e4b` → `gemma4-e2b`, same fallback chain as `senkani_vision` and `GemmaInferenceAdapter`), runs text-only inference under `MLXInferenceLock.shared` with the memory-pressure unload hook, and assembles a role-prefixed prompt (`[<role>] <content>` per message) so Gemma sees turn boundaries. When MCP is NOT started in-process — pure CLI `senkani serve --openai`, unit tests — the listener falls back to the V.13a-3 placeholder and logs `openai-serve chat_backend=placeholder` once at startup so test environments stay self-contained. Non-streaming surface only; the SSE token-by-token streaming path, `model_not_available` readiness 503, tokenizer-accurate `usage` counts, and audit-chain regression sweep continue under V.13 sub-items 2-4. Core stays MLX-free (`grep -rE '^import MLX' Sources/Core/` returns empty); CLI install size unchanged.
- **Real token-by-token Gemma 4 SSE streaming (Phase V.13 chat-backend-2, 2026-05-30).** A `stream: true` request now emits SSE content deltas as MLX produces each token, not as a post-hoc split over a one-shot completion. New `public protocol StreamingChatEngine: Sendable` (`Sources/Core/OpenAIEndpoint/StreamingChatEngine.swift`) returns `AsyncThrowingStream<OpenAIChatHandler.TokenDelta, Error>` — the first async-stream shape in the OpenAIEndpoint surface; the MCP-side `MLXStreamingChatEngineAdapter` (`Sources/MCP/ChatHandlerRegistration.swift`) drives a new `MLXChatEngine.stream(prompt:)` that yields each `Generation.chunk` to the continuation as MLX's `container.generate` produces it (clients accumulate `delta.content` verbatim — the streaming path skips end-of-stream trimming because it can't unwrite earlier deltas). `OpenAIChatHandler.renderStreamingEvents(...)` translates `TokenDelta` deltas into pre-encoded SSE chunks (role chunk + content chunks + terminal `finish_reason: "stop"`); `OpenAIChatStream.Plan` gains a `streamingEvents:` async-source init carried through a new `driveStreaming(...)` channel pump that bridges the async source to the listener's synchronous `Sink` (peer disconnect → producer Task cancel; producer error → `.clientCancel`). `ServeCommand` routes non-tool-use content streams through the registered handler when present, with an NSLock-protected accumulator teeing each delta so the `chatStream` audit entry's `completion_token_count` reflects bytes actually streamed; tool-use streams (V.13d-1's `tool_calls` delta path) and unregistered-handler runs keep the V.13b collected-then-chunk fallback. Core stays MLX-free (`grep -rE '^import MLX' Sources/Core/` returns empty); the wire shape is unchanged from V.13b (`text/event-stream` head, `data: {…}\n\n` events, `data: [DONE]\n\n` sentinel) — only the arrival timing differs. Non-local-tier 503s, readiness 503, tokenizer-accurate `usage` counts, audit-chain regression sweep + phase row flip continue under V.13 sub-items 3-4.
- **OpenAI-compatible streaming tool-calls (Phase V.13d-1).** A `stream: true` request that resolves to a tool call now emits OpenAI-shaped `choices[].delta.tool_calls[]` SSE fragments instead of dropping the call into an empty content stream (the V.13d deferral). The first fragment for a tool call carries `{ index, id, type: "function", function: { name, arguments: "" } }`; subsequent fragments append `function.arguments` string pieces that concatenate byte-for-byte back to the full JSON-encoded arguments string; a terminal chunk carries `finish_reason: "tool_calls"` before the `data: [DONE]` sentinel. The tool-call path reuses the same `OpenAIChatStream` chunk model, `run(…)` drive loop, and `splitForStreaming` splitter as the content path — **no parallel streaming stack** — with a new `toolCallEvents(…)` builder and a sparse `Chunk.Delta.ToolCallFragment` (`index` always present so a client keys fragments by array index). Scope-gate parity holds: an out-of-scope or malformed `stream: true` tool-use request is refused with a framed `403`/`400` *before* any SSE byte (no half-open stream), and each streamed tool-use request still appends exactly one tamper-evident audit-chain entry (`status` distinguishing `ok` vs `client_cancel`).
...
| Release commitments | cold-start < 250 ms p95 · idle memory < 75 MB · install size < 50 MB · classifier latency < 2 ms p95 (slot pending U.1) · OpenAI-endpoint cold-start < 1500 ms p95 |
| Terminal render | p50 ~2 ms, p99 ~3.4 ms |
| Filter throughput | > 10 k lines/sec |
| Symbol search | < 5 ms cold, < 1 ms cached |
| Secret scan | < 2 ms / KB |
| Unit tests | **2577 passing** |
