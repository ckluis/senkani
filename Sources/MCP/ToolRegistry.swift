                ...
                schema: Tool(
                    ...
                    inputSchema: .object([
                        ...
                        "properties": .object([
                            ...
                            "dispatch": .object(["type": .string("string"), "enum": .array([.string("subprocess"), .string("headless")]), "description": .string("U.2b-1b-6 — runner selector. 'subprocess' (default) drives the node+Playwright Chromium subprocess. 'headless' drives the off-screen WKWebView runner registered via BrowserDispatchRegistry by the SenkaniApp host at startup; without that registration (standalone senkani-mcp binary) the dispatcher returns a structured headless_not_yet_implemented refusal. Audit-chain rows carry runner=<value> in both modes (subprocess → 'subprocess'; headless → 'wkwebview-headless').")]),
